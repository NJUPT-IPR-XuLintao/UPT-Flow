...
def main():
    ...
    parser.add_argument("--opt", default=".\confs/UPT-Flow.yml")
    ...
    for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
        ...
        # You can follow a similar way of 'Kind' to finetune the overall brightness as illustrated in Line 73 (https://github.com/zhangyhuaee/KinD/blob/master/evaluate_LOLdataset.py).
        ...
        'test过程中计算mean，进行矫正，指标会更高'
        # mean_out = sr_t.view(sr_t.shape[0],-1).mean(dim=1)
        # mean_gt = hr.astype(np.float32).mean()/255
